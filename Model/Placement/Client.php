...
namespace Clearpay\Clearpay\Model\Placement;
    ...
    public function execute(string $mpid): array
    {
        ...
        try {
            ...
            if (is_string($this->client->getBody()) && strpos($this->client->getBody(), 'error code') !== false ) {
                $this->logger->critical("Clearpay: Error fetching placement data for mpid: {$mpid}");
                return [];
            }
            ...
            $this->logger->critical("Clearpay: Error fetching placement data for mpid: {$mpid}. " . $message, $e->getTrace());
        } finally {
