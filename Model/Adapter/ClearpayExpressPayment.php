...
class ClearpayExpressPayment
{
     ...
     *
     */
    public function getClearPayExpressOrderToken($clearpayOrderToken, $payment, $targetObject)
    {
        if ($targetObject && $targetObject->getReservedOrderId()) {
            $result = $clearpayOrderToken->generate($targetObject, [
                ...
                'mode' => \Clearpay\Clearpay\Model\Config\Source\CartMode::EXPRESS_CHECKOUT
            ]);
        } elseif ($targetObject) {
            $result = $clearpayOrderToken->generate($targetObject, [
                'mode' => \Clearpay\Clearpay\Model\Config\Source\CartMode::EXPRESS_CHECKOUT
    ...
    public function setOrderData($data)
    {
        ...
        if (! $quote->isVirtual()) {
            ...
            ->setPaymentMethod(\Clearpay\Clearpay\Model\Payovertime::METHOD_CODE);
        } else {
            $billingAddress->setPaymentMethod(\Clearpay\Clearpay\Model\Payovertime::METHOD_CODE);
        }
