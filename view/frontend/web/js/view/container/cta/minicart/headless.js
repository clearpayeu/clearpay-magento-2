(function (d, w, s) {
    ...
    function fetchConfigData() {
        const query = `mutation {
             getClearpayConfigMiniCart(input: {
                 cart_id: "${cartId}"
                 store_id: "${storeId}"
             }) {
                 allowed_currencies
        ...
        return fetch(graphqlEndpoint, requestOptions)
            ...
            .then(data => {
                if (data && !data.errors) {
                    const clearpayConfig = data.data.getClearpayConfigMiniCart;
                    let isEnabledCtaMinicart = clearpayConfig.is_enabled_cta_minicart_headless;
