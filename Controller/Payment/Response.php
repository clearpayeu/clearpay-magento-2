...
class Response extends \Magento\Framework\App\Action\Action
{
     ...
     * @param \Clearpay\ClearpayEurope\Model\Response $response
     * @param \Clearpay\ClearpayEurope\Helper\Data $helper
     * @param \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderDirectCapture $directCapture
     * @param \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderAuthRequest $authRequest
     * @param \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderTokenCheck $tokenCheck
     * @param \Magento\Framework\Json\Helper\Data $jsonHelper
     * @param \Clearpay\ClearpayEurope\Model\Config\Payovertime $clearpayConfig
     ...
     * @param \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderPaymentCapture $paymentCapture
     */
    public function __construct(
        \Magento\Framework\App\Action\Context $context,
        \Magento\Framework\Controller\Result\ForwardFactory $resultForwardFactory,
        \Magento\Checkout\Model\Session $checkoutSession,
        \Clearpay\ClearpayEurope\Model\Response $response,
        \Clearpay\ClearpayEurope\Helper\Data $helper,
        \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderDirectCapture $directCapture,
        \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderAuthRequest $authRequest,
        \Clearpay\ClearpayEurope\Model\Adapter\V2\ClearpayOrderTokenCheck $tokenCheck,
    ...
    {
        // debug mode
        $this->_helper->debug('Start \Clearpay\ClearpayEurope\Controller\Payment\Response::execute() with request ' . $this->_jsonHelper->jsonEncode($this->getRequest()->getParams()));
        ...
        // debug mode
        $this->_helper->debug('Finished \Clearpay\ClearpayEurope\Controller\Payment\Response::execute()');
        ...
        try {
            switch ($query['status']) {
                ...
                case \Clearpay\ClearpayEurope\Model\Response::RESPONSE_STATUS_SUCCESS:
					...
					if(!$this->_helper->getConfig('payment/clearpayeupayovertime/payment_flow') || $this->_helper->getConfig('payment/clearpayeupayovertime/payment_flow')=="immediate" || $quote->getIsVirtual()){
                    ...
                    if (empty($response['status'])) {
                        $response['status'] = \Clearpay\ClearpayEurope\Model\Response::RESPONSE_STATUS_DECLINED;
                        $this->_helper->debug("Transaction Exception (Empty Response): " . json_encode($response));
                    }

                    switch ($response['status']) {
                        case \Clearpay\ClearpayEurope\Model\Response::RESPONSE_STATUS_APPROVED:
							$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::ADDITIONAL_INFORMATION_KEY_ORDERID, $response['id']);
                            
							$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::PAYMENT_STATUS,$response['status']);
							
							if($response['status']==\Clearpay\ClearpayEurope\Model\Response::PAYMENT_STATUS_AUTH_APPROVED && array_key_exists('events',$response)){
								try{
									$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::AUTH_EXPIRY,$this->_timezone->date($response['events'][0]['expires'])->format('Y-m-d H:i T'));
								}
								catch(\Exception $e){
									$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::AUTH_EXPIRY,$this->_timezone->date($response['events'][0]['expires'],null,false)->format('Y-m-d H:i T'));
									$this->_helper->debug($e->getMessage());
								}
							}
							
							$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::OPEN_TOCAPTURE_AMOUNT, array_key_exists('openToCaptureAmount',$response) && !empty($response['openToCaptureAmount']) ? $response['openToCaptureAmount']['amount'] : "0.00");
	                          ...
	                          try{
		                         ...
		                         if (preg_match('/SQLSTATE\[40001\]: Serialization failure: 1213 Deadlock found/', $e->getMessage()) && $tries<2){
			                       ...
			                       sleep(1);
									...
									$paymentStatus = $payment->getAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::PAYMENT_STATUS);
									
									if($paymentStatus == \Clearpay\ClearpayEurope\Model\Response::PAYMENT_STATUS_AUTH_APPROVED){
                            ...
                            if ($order) {
								...
								if($payment->getAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::PAYMENT_STATUS)==\Clearpay\ClearpayEurope\Model\Response::PAYMENT_STATUS_AUTH_APPROVED){
									$totalDiscount = $this->_calculateTotalDiscount($order);
									if($totalDiscount > 0){
										$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::ROLLOVER_DISCOUNT,$this->_calculateTotalDiscount($order));
									}
                            ...
                            break;
                        case \Clearpay\ClearpayEurope\Model\Response::RESPONSE_STATUS_DECLINED:
		...
		if($totalCaptureAmount >= 1){
			...
			if(!array_key_exists("errorCode",$response)) {
				...
				if(array_key_exists('openToCaptureAmount',$response) && !empty($response['openToCaptureAmount'])){
					$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::OPEN_TOCAPTURE_AMOUNT,$response['openToCaptureAmount']['amount']);
				}
			}
			else{
			...
			if($totalCaptureAmount < 1 && $totalCaptureAmount > 0){
				$payment->setAdditionalInformation(\Clearpay\ClearpayEurope\Model\Payovertime::ROLLOVER_AMOUNT, number_format($totalCaptureAmount, 2, '.', ''));
			}
		}
